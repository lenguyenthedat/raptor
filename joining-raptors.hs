...
main = do
        ...
        let raptorsCombined = joinList raptorsCountry
        outh                <- openFile (resultFile) WriteMode
        hPutStrLn outh $ intercalate "\n" $ map (\x -> country++'\t': intercalate "\t" (take sku_count x)) $ toRaptor raptorsCombined
        hClose outh

join :: Eq a => [[a]] -> [[a]] -> [[a]]
join (ks1:f1) (ks2:f2) = union ks1 ks2 : [k1 : v1++v2 | (k1:v1) <- f1, (k2:v2) <- f2, k1 == k2]
join _        _        = []
...
scoreSKU_to_tuple s | isInfixOf "-" s = let splits = splitOn "-" s in (read (splits !! 0) :: Float, splits !! 1 )
                    | otherwise = (0,"")
